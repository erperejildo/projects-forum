...
- When creating or refactoring components that require icons, use Heroicons as the primary source: https://heroicons.com/
- Prefer copying the SVG for the icon you need from Heroicons and inlining it in the component template, or add the SVG to `src/assets/icons/` and reference it. Use the Outline or Solid style consistently per component.
- Ensure icons are accessible: provide `aria-hidden="true"` for decorative SVGs and `aria-label` on interactive icon buttons.
